...
def johnson(n, S):
    ...
    # First create G' by adding a new vertex s and edges (s, v) of weight 0
    Adj = [[] if v < n else list(range(n)) for v in range(n + 1)]
    Weights = {(n, v): 0 for v in range(n)}
    for u, v, w in S:
        Weights[(u, v)] = w
        Adj[u].append(v)

    # Create a function w(u, v) that returns the weight of the edge from u to v
    def w(u, v):
        return Weights[(u, v)]

    # Run Bellman-Ford on G' to get the shortest path from s to every vertex,
    # or abort if a negative-weight cycle is detected
    if (bf := bellman_ford(Adj, w, n)) is None:
        return None
    else:
        h, _ = bf

    # Reweight the edges of G' using the values of h
    for u, v in Weights.keys():
        Weights[(u, v)] += h[u] - h[v]

    # Run Dijkstra's algorithm on G' for each vertex v to get the shortest path
    for u in range(n):
